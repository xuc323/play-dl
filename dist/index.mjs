...
import { createGunzip, createBrotliDecompress, createDeflate } from "node:zlib";
...
function uploadCookie() {
  if (youtubeData.cookie && youtubeData.file)
    writeFileSync(".data/youtube.data", JSON.stringify(youtubeData, void 0, 4));
}
...
function download_url(format, sig) {
  ...
  const parsed_url = new URL2(decoded_url);
...
var LiveStream = class {
  ...
  constructor(dash_url, interval, video_url, precache) {
    this.stream = new Readable({ highWaterMark: 5 * 1e3 * 1e3, read() {
    } });
  ...
  async initialize_dash() {
    ...
    await request_stream(`https://${new URL4(this.base_url).host}/generate_204`);
    if (this.sequence === 0) {
...
var WEB_ELEMENT_KEYS = Object.keys(WebmElements);
var WebmSeeker = class extends Duplex {
  remaining;
  state;
  chunk;
  cursor;
...
async function stream_from_info(info, options = {}) {
  ...
  await request_stream(`https://${new URL5(final[0].url).host}/generate_204`);
...
var spotifyData;
if (existsSync2(".data/spotify.data")) {
  spotifyData = JSON.parse(readFileSync2(".data/spotify.data", "utf-8"));
...
async function SpotifyAuthorize(data, file) {
  ...
  if (file)
    writeFileSync2(".data/spotify.data", JSON.stringify(spotifyData, void 0, 4));
  else {
...
var SoundCloudStream = class {
  ...
  constructor(url, type = "arbitrary" /* Arbitrary */) {
    this.stream = new Readable2({ highWaterMark: 5 * 1e3 * 1e3, read() {
    } });
    this.type = type;
    this.url = url;
...
// play-dl/SoundCloud/index.ts
var soundData;
if (existsSync3(".data/soundcloud.data")) {
  soundData = JSON.parse(readFileSync3(".data/soundcloud.data", "utf-8"));
}
...
import { existsSync as existsSync4, mkdirSync, writeFileSync as writeFileSync3 } from "node:fs";
...
`);
        ...
        ask.question("Client ID : ", async (id) => {
          ...
          if (await check_id(client_id)) {
            ...
            writeFileSync3(".data/soundcloud.data", JSON.stringify({ client_id }, void 0, 4));
          } else
        ...
        ask.question("Cookies : ", (cook) => {
          ...
          writeFileSync3(".data/youtube.data", JSON.stringify({ cookie }, void 0, 4));
          ask.close();
        });
      } else {
        console.log("That option doesn't exist. Try again...");
...
export {
  ...
  spotify,
  stream3 as stream,
  stream_from_info3 as stream_from_info,
  validate,
  video_basic_info,
  video_info,
  yt_validate
